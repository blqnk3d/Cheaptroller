...
// Toggle: allow frontend config or not
let ALLOW_FRONTEND_CONFIG = false;
...
if(fs.existsSync(SETTINGS_FILE)) {
    ...
    } catch(e){ console.error('❌ Failed to load persisted config:', e); }
}
...
const normalize = (x,y) => { const m = magnitude(x,y); return m?[x/m,y/m]:[0,0]; };
...
function mapDirectionToKeysFromAxes(ax, ay) {
    const keys = new Set();
    if(ay<-dynamicConfig.DEADZONE) keys.add('w'); else if(ay>dynamicConfig.DEADZONE) keys.add('s');
    if(ax<-dynamicConfig.DEADZONE) keys.add('a'); else if(ax>dynamicConfig.DEADZONE) keys.add('d');
    return keys;
}

function updatePressedDirectionKeys(side,newKeys){
    const now=nowMs();
    for(const key of Array.from(pressedDirectionKeys)) if(!newKeys.has(key) && now-lastKeyToggleTime>30){ robot.keyToggle(key,'up'); pressedDirectionKeys.delete(key); lastKeyToggleTime=now; }
    for(const key of newKeys) if(!pressedDirectionKeys.has(key) && now-lastKeyToggleTime>30){ robot.keyToggle(key,'down'); pressedDirectionKeys.add(key); lastKeyToggleTime=now; }
}
...
server.bind(UDP_PORT,'0.0.0.0',()=>{ console.log(`🟢 UDP running on ${UDP_PORT}`); });
...
httpServer.listen(WEB_PORT,()=>console.log(`🌐 Web server running on http://localhost:${WEB_PORT}`));
