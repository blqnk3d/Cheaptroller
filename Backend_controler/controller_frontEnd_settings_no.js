...
let frontendConfigState = { allowFrontendConfig: true };
if (fs.existsSync(SETTINGS_FILE)) {
    try {
        const data = JSON.parse(fs.readFileSync(SETTINGS_FILE));
        Object.assign(dynamicConfig, data);
        if (typeof data.allowFrontendConfig === 'boolean') frontendConfigState.allowFrontendConfig = data.allowFrontendConfig;
        console.log('⚡ Loaded persisted config:', dynamicConfig, 'Frontend config:', frontendConfigState.allowFrontendConfig);
    } catch (e) {
        console.error('❌ Failed to load persisted config:', e);
    }
}

let ALLOW_FRONTEND_CONFIG = frontendConfigState.allowFrontendConfig;
...
function saveConfig() {
    try {
        fs.writeFileSync(
            SETTINGS_FILE,
            JSON.stringify({ ...dynamicConfig, allowFrontendConfig: ALLOW_FRONTEND_CONFIG }, null, 2)
        );
    } catch (e) {
...
udpServer.bind(UDP_PORT, '0.0.0.0', () => console.log(`🟢 UDP running on ${UDP_PORT}`));
...
app.post('/api/toggleFrontendConfig', (req, res) => {
    if (typeof req.body.allow === 'boolean') {
        ALLOW_FRONTEND_CONFIG = req.body.allow;
        saveConfig(); // persist toggle
        console.log('⚡ ALLOW_FRONTEND_CONFIG =', ALLOW_FRONTEND_CONFIG);
        res.json({ success: true, allowFrontendConfig: ALLOW_FRONTEND_CONFIG });
    } else {
...
httpServer.listen(WEB_PORT, () => console.log(`🌐 Web server running on http://localhost: ${getAllLocalIPs().join("|")} ${WEB_PORT}`));
